...
public class ObjReader {
    ...
    private HalfEdgeTriangleMesh currentMesh = null;
    private TriangleMesh currentTriangleMesh = null;
    private Material currentMaterial = null;
    private Map<String, Material> materials = null;
    private List<HalfEdgeTriangleMesh> meshes = new ArrayList<HalfEdgeTriangleMesh>();
    private List<TriangleMesh> triangleMeshes = new ArrayList<TriangleMesh>();
    private int vertexIndexOffset = 0;
    private int texCoordOffset = 0;
    private ITriangleMeshFactory meshFactory = null;
     ...
     */
    public List<HalfEdgeTriangleMesh> read(ITriangleMeshFactory meshFactory, final String filename) {
        // Setup
        this.meshFactory = meshFactory;
        meshes.clear();
        directory = new File(filename).getParent() + "/";
        currentMesh = meshFactory.createMesh();
        ...
        // Post-process meshes
        for (Iterator<HalfEdgeTriangleMesh> it = meshes.iterator(); it.hasNext(); ) {
...
//        }
    ...
    public List<TriangleMesh> readTriangleMesh(ITriangleMeshFactory meshFactory, final String filename) {
        // Setup
        this.meshFactory = meshFactory;
        triangleMeshes.clear();
        directory = new File(filename).getParent() + "/";
        currentTriangleMesh = meshFactory.createTriangleMesh();
        triangleMeshes.add(currentTriangleMesh);
        materials = new HashMap<String, Material>();
        vertexIndexOffset = 0;
        texCoordOffset = 0;
        //textureCoordinates.clear();
        ...
        // Post-process meshes
        for (Iterator<TriangleMesh> it = triangleMeshes.iterator(); it.hasNext(); ) {
            ITriangleMesh mesh = it.next();
            if (mesh.getNumberOfTriangles() == 0) {
                it.remove();
                continue;
...
//        }
    ...
    private void parseTLine(String strLine) {
        ...
        if (operator.equals(OP_NEW_OBJECT)) {
            // Neues (Teil-)Objekt
            vertexIndexOffset += currentTriangleMesh.getNumberOfVertices();
            texCoordOffset += currentTriangleMesh.getNumberOfTextureCoordinates();
            currentTriangleMesh = meshFactory.createTriangleMesh();
            ...
            if (components.length > 1) {
                ...
                if (mat != null) {
                    currentMaterial = mat;
                    currentTriangleMesh.setTextureName(null);
                    if (mat.getTextureFilename() != null
                            && mat.getTextureFilename().length() > 0) {
                        currentTriangleMesh.setTextureName(mat.getTextureFilename());
                    }
                }
            }
            ...
            if (position != null) {
                currentTriangleMesh.addVertex(position);
            }
            ...
            if (t != null) {
                currentTriangleMesh.addTextureCoordinate(t);
     ...
     */
    private void parseTFacet(String strLine) {
    ...
    private void parseFacet(String strLine) {
        ...
        if (allCoords.length == 4) {
            createFacet(allCoords[1], allCoords[2], allCoords[3]);

        } else if (allCoords.length == 5) {

            createFacet(allCoords[1], allCoords[2], allCoords[3]);

            createFacet(allCoords[1], allCoords[3], allCoords[4]);
        }
    ...
    private void createTriangle(String token1, String token2, String token3) {
        ...
        AbstractTriangle t = meshFactory.createTriangle();

        t.setVertexIndices(getVertexIndexFromToken(token1),
        ...
        currentTriangleMesh.addTriangle(t);

    }

    private void createFacet(String token1, String token2, String token3) {

        AbstractTriangle t = meshFactory.createFacet();
